...
class CryptoRiskScore(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X: pd.DataFrame):
        ...
        price_data = X[self.price_columns].apply(pd.to_numeric, errors="coerce").fillna(0)
        price_volatility = price_data.std(axis=1)
        
        # Calculate funding rate risk (absolute values)
        funding_data = X[self.funding_columns].apply(pd.to_numeric, errors="coerce").fillna(0)
...
FEATURE_TRANSFORMERS = {
    "crypto_risk_score": lambda config: CryptoRiskScore(
        ...
        funding_columns=[col for col in config.get("raw_features", []) if "_funding_rate" in col]
    ),
