...
def _html_from_report(report: dict) -> str:
    ...
    if missing_summary:
        ...
        lines.append("<ul>")
        lines.append(f"<li>Missing values before handling: {missing_summary.get('missing_before', 0)}</li>")
        if strategy == 'impute':
            lines.append(f"<li>Total values imputed: {missing_summary.get('total_imputed', 0)}</li>")
        elif strategy == 'drop':
            lines.append(f"<li>Rows dropped: {missing_summary.get('rows_dropped', 0)}</li>")
        lines.append("</ul>")
    ...
    if details:
        ...
        for d in details.values():
            keys.update(d.keys())
        columns = ["column", "status", "expected_type", "sample_values"] + sorted(list(keys - {"status", "expected_type", "sample_values"}))
        ...
        for i, (col, d) in enumerate(details.items()):
            row_style = "background-color: #ffffff;" if i % 2 == 0 else "background-color: #f9f9f9;"
            lines.append(f"<tr style='{row_style}'>")
            for c in columns:
                val = col if c == "column" else d.get(c, "")
                if isinstance(val, list):
                    val = ", ".join(map(str, val)) # Join list items for display
                lines.append(f"<td style='padding: 8px; border: 1px solid #ddd;'>{val}</td>")
            lines.append("</tr>")
        lines.append("</table>")
...
def run_data_validation(input_artifact: str):
    ...
    with mlflow.start_run(run_name="data_validation") as mlrun:
        ...
        output_path = os.path.join(project_root, config.get("data_source", {}).get("processed_path"))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input-artifact", type=str, required=True, help="Path to the raw data CSV artifact.")
