...
def date_to_ms(date_str: str) -> int:
    ...
    try:
        ...
        logger.error(f"Error converting date '{date_str}' to milliseconds: {e}")
        raise
        ...
        while params["startTime"] < end_ms:
            try:
                ...
                if request_count % 10 == 0:
                    logger.debug(f"[{symbol}] Fetched {len(rows)} records so far")
    ...
    try:
        ...
        while params["startTime"] < params["endTime"]:
            try:
                ...
                if request_count % 10 == 0:
                    logger.debug(f"[{symbol}] Fetched {len(rows)} funding records so far")
                ...
                logger.error(f"[{symbol}] Unexpected error during funding fetch: {e}")
                break

        if not rows:
            logger.warning(f"[{symbol}] No funding data fetched")
            return pd.DataFrame(columns=["timestamp", f"{symbol}_funding_rate"])

        logger.info(f"[{symbol}] Successfully fetched {len(rows)} funding rates")
    ...
    try:
        ...
        for symbol in SYMBOLS:
            try:
                ...
                funding_df = fetch_binance_funding_rate(symbol, start_date, end_date)
