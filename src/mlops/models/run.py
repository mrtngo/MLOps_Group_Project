...
def run_model_training(input_artifact_dir: str):
    ...
    try:
        with mlflow.start_run(run_name="model_training") as mlrun:
            ...
            X_train_reg = pd.read_csv(os.path.join(input_artifact_dir, "X_train_reg.csv"))
            y_train_reg = pd.read_csv(os.path.join(input_artifact_dir, "y_train_reg.csv")).squeeze()
            
            X_train_class = pd.read_csv(os.path.join(input_artifact_dir, "X_train_class.csv"))
            y_train_class = pd.read_csv(os.path.join(input_artifact_dir, "y_train_class.csv")).squeeze()
            ...
            class_model = model_trainer.train_logistic_regression(X_train_class, y_train_class)
            ...
            reg_params = config.get("model", {}).get("linear_regression", {}).get("params", {})
            ...
            class_params = config.get("model", {}).get("logistic_regression", {}).get("params", {})
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run the model training pipeline step.")
    config = load_config("conf/config.yaml")
