    ...
    with mlflow.start_run(run_name="model_evaluation") as mlrun:
        ...
        if "linear_regression" in model_artifact_path:
            ...
            if reg_metrics:
                ...
                mlflow.log_metrics({f"regression_eval_{k}": v for k, v in reg_metrics.items()})
            ...
            if class_metrics:
                ...
                mlflow.log_metrics({f"classification_eval_{k}": v for k, v in class_metrics.items()})
                wandb.log({f"classification_eval_{k}": v for k, v in class_metrics.items()})
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test-data-path", type=str, required=True, help="Path to the test data directory.")
