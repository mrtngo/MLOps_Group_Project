...
def test_main_all_stage(monkeypatch, dummy_df):
    ...
    monkeypatch.setattr("mlops.main.define_features_and_label", lambda: feature_return)
    monkeypatch.setattr("mlops.main.create_price_direction_label", lambda df, label: df)
    monkeypatch.setattr(
        "mlops.main.train_model",
        lambda df: ("regression_model", "classification_model"),
    )
    metrics_return = ({"RMSE": 1.23}, {"Accuracy": 0.9, "ROC AUC": 0.88})
    monkeypatch.setattr("mlops.main.evaluate_models", lambda df: metrics_return)
...
def test_main_infer_stage(monkeypatch, dummy_df):
    ...
    monkeypatch.setattr(
        "mlops.main.load_config",
        lambda _: {"data_validation": {"schema": {"columns": []}}},
    )
